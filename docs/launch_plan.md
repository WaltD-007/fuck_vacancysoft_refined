...
| C15 | Intelligence model quality upgrade: dossier gpt-5-mini → gpt-5.2 (reasoning medium → low to avoid double-paying for quality); campaign gpt-5.2 → gpt-5.4 (writing quality was the new ceiling). Timeout bumped 300s → 450s. `PROMPT_VERSION` bumped v1.1 → v1.2 as a cost_report segmentation marker. Est per-lead cost $0.08 → $0.135 (still inside $0.15 envelope). Rollback: flip config back + restart API + worker (no code deploy) | ✅ PR #10 (commit c0969eb) |
| C16 | Campaign prompt v2: operator-authored shape where "tone controls content" — each of the six tones has a fixed home dossier section and one concrete anchor persisting across all five sequences (six distinct five-email arcs vs the prior "same message, different voice" model). Legacy v1 preserved behind `campaign_template_version = "v1"` config flag; resolver selects on each call via `_load_intel_config()`. No cost impact (campaign prompt token count ~unchanged). Rollback: flip flag + restart API + worker | ✅ PR #11 (commit ab4b825) |
| C17 | HM candidate cap 3 → 6 across all three HM prompt surfaces (dossier main prompt, OpenAI web_search fallback, SerpApi extraction system + user). Anti-padding clause tells the model to lead with highest-confidence and stop short if only 2–3 are genuinely plausible. UI HM list gets inner `maxHeight: 200 + overflowY: auto` so the outer lead card's size is unchanged. Rollback: `git revert` the single commit | ✅ PR #12 (commit 671352c) |
| C18 | Paste-lead endpoint: new `POST /api/leads/paste` takes a single URL, runs LinkedIn-URL gate + recruiter-filter + Playwright scrape + classifier + dedupe, then enqueues `process_lead`. Also ships a `_resolve_company()` guard in `dossier.py` that stops `(Manual Paste)` being substituted literally into dossier prompts + HM SerpApi queries (was causing gpt-4o-mini to fabricate HMs from training data). Supporting infra: new `intelligence/url_scrape.py` centralises the Playwright runner call across three callers. Rollback: `git revert` the merge commit — no schema change | ✅ PR #13 (commit b04f255) |
| C19 | Campaign v2 revision: operator-authored "offer of value" global rule. Every email must close with one concrete offer the sender is prepared to give (call, shortlisted CV, market briefing, salary benchmark, etc.) — never a vague "happy to chat". Offers must vary across the five sequences within each tone-arc. Compatible with the existing "don't ask the reader for more information" rule (offering ≠ asking). v1 stays frozen so rollback via `campaign_template_version = "v1"` still works | ✅ PR #14 (commit d3a4116) |
| C20 | Dossier shape + cost tune (v1.2 → v1.3): `dossier_search_context_size` dropped "high" → "medium" (saves ~$0.017/lead on prompt tokens by halving web_search-injected context); `stated_vs_actual` cap raised EXACTLY-2 → UP TO 4 rows; `spec_risk` cap raised 1-2 → UP TO 4 items; anti-padding guards added to both sections. Resolver slices bumped `[:3]` → `[:4]` so the campaign prompt sees every row the dossier produced. `PROMPT_VERSION` bumped v1.2 → v1.3 as cost_report segmentation marker. Net per-lead cost ~$0.182 → ~$0.170 (richer output + cheaper input). Rollback: flip config back + restart + revert | ✅ PR #15 (commit a6aa69e) |
| C21 | Teamtailor source cleanup: 164 active `adapter_name='teamtailor'` rows triaged. Of those, 148 were cross-adapter duplicates (same employer already had an active row under `generic_site`/`workday`/`greenhouse`/`eightfold`) — symptom: 93% of teamtailor sources in the 2026-04-21 02:00 run failed with 404s / ParseErrors because the adapter was appending `/jobs.rss` to URLs that weren't Teamtailor boards. Actions: **148 deactivated** (cross-dupes), **5 deactivated** (internal same-`*.teamtailor.com`-host dupes), **5 moved** to `adapter_name='generic_site'` (Cinctive Capital, Cynergy Bank, Eisler Capital, Hartree Partners, Savills IM), **6 kept** as teamtailor (BACB, Coller Capital, GHIB, GraviteeSource, Hinckley & Rugby, IMPOWER Consulting — all genuine `*.teamtailor.com` hosts). Net active teamtailor: 164 → 6. Backup: `.data/backups/sources_pre_teamtailor_cleanup_20260421-0906.sql`. Review CSV + rollback-safe `apply.sql` at `~/Desktop/teamtailor_cleanup/`. Same shape as C13 Lever cleanup | ✅ DB-only (pure SQL UPDATE, no code change) |
