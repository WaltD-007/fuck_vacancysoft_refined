...
NETWORK_HINTS = ("job", "requisition", "career", "search")
...
def _make_record(job: dict[str, Any], board: dict[str, Any], source: str) -> DiscoveredJobRecord | None:
    title = _clean(job.get("title") or job.get("jobTitle") or job.get("name") or job.get("requisitionTitle"))
    url = _clean(job.get("url") or job.get("jobUrl") or job.get("applyUrl"))
    if url and not url.startswith("http"):
        url = urljoin(str(board.get("url") or ""), url)
    if not title and not url:
        return None
    location = _clean(job.get("location") or job.get("city") or job.get("workLocation"))
    posted_at = _clean(job.get("postedDate") or job.get("postedAt") or job.get("updatedAt"))
    company_name = lookup_company("adp", board_url=board.get("url"), explicit_company=board.get("company"))
    ...
    return DiscoveredJobRecord(
        ...
        provenance={
            ...
            "platform": "ADP Recruiting Management",
...
def _walk(node: Any, board: dict[str, Any], source: str) -> list[DiscoveredJobRecord]:
    ...
    if isinstance(node, dict):
        if any(k in node for k in ("jobTitle", "title", "applyUrl", "jobUrl", "requisitionTitle")):
...
class ADPAdapter(SourceAdapter):
    adapter_name = "adp"
    ...
    async def discover(self, source_config: dict[str, Any], cursor: str | None = None, since: datetime | None = None, on_page_scraped: PageCallback = None) -> DiscoveryPage:
        ...
        if not board_url:
            raise ValueError("ADPAdapter requires job_board_url")
        diagnostics = AdapterDiagnostics(metadata={"board_url": board_url})
        if cursor is not None:
            diagnostics.warnings.append("ADPAdapter does not support pagination. cursor was ignored.")
        if since is not None:
            diagnostics.warnings.append("ADPAdapter does not enforce incremental sync at source. since was ignored.")
        ...
        try:
            ...
            diagnostics.errors.append(f"ADP page timeout: {exc}")
            raise
        except PlaywrightError as exc:
            diagnostics.errors.append(f"ADP browser failure: {exc}")
            raise
